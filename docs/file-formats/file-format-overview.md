...
- **Starfield** - Creation Engine 2; geometry-less NIF (`BSGeometry` → external `.mesh`), layered `.mat` materials, component-based plugins, in-house animation. See the dedicated **[Starfield](../game-specific/starfield/starfield-overview.md)** section — its formats differ enough to stand apart from the pages above.
